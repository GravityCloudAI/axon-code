...
func TestLoadThemesFromJSON(t *testing.T) {
	...
	expectedThemes := []string{"tokyonight", "axoncode", "everforest", "ayu"}
...
func TestLoadThemesFromDirectories(t *testing.T) {
	...
	os.MkdirAll(filepath.Join(userConfig, "axoncode", "themes"), 0755)
	os.MkdirAll(filepath.Join(projectRoot, ".axoncode", "themes"), 0755)
	os.MkdirAll(filepath.Join(cwd, ".axoncode", "themes"), 0755)
	...
	os.WriteFile(filepath.Join(userConfig, "axoncode", "themes", "override-test.json"), []byte(testTheme1), 0644)
	os.WriteFile(filepath.Join(projectRoot, ".axoncode", "themes", "override-test.json"), []byte(testTheme2), 0644)
	os.WriteFile(filepath.Join(cwd, ".axoncode", "themes", "override-test.json"), []byte(testTheme3), 0644)
