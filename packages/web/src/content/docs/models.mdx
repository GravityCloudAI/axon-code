...
Most popular providers are preloaded by default. If you've added the credentials for a provider through `axoncode auth login`, they'll be available when you start axoncode.
...
Here are the ones we recommend with axoncode:
...
```jsonc title="axoncode.jsonc" {7-12,19-24}
{
  "$schema": "https://axoncode.ai/config.json",
  "provider": {
    "openai": {
...
When axoncode starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the axoncode config.
