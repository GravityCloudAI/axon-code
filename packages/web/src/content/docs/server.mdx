---
title: Server
description: Interact with axoncode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `axoncode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an axoncode client can use.
...
```bash
axoncode serve [--port <number>] [--hostname <string>]
```
...
When you run `axoncode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the axoncode server to interact with axoncode programmatically.
:::

This architecture lets axoncode support multiple clients and allows you to interact with axoncode programmatically.

You can run `axoncode serve` to start a standalone server. If you have the
axoncode TUI running, `axoncode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the axoncode [IDE](/docs/ide) plugins.
...
The axoncode server exposes the following APIs.
...
| `POST`   | `/session/:id/message`                   | Send chat message                  | body matches [`ChatInput`](https://github.com/sst/axoncode/blob/main/packages/axoncode/src/session/index.ts#L358), returns <a href={typesUrl}><code>Message</code></a>     |
| `POST`   | `/session/:id/shell`                     | Run a shell command                | body matches [`CommandInput`](https://github.com/sst/axoncode/blob/main/packages/axoncode/src/session/index.ts#L1007), returns <a href={typesUrl}><code>Message</code></a> |
