---
title: Providers
description: Using any LLM provider in axoncode.
---

import config from "../../../config.mjs"
export const console = config.console

axoncode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support for **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using `axoncode auth login`.
2. Configure the provider in your axoncode config.

---

### Credentials

When you add a provider's API keys with `axoncode auth login`, they are stored
in `~/.local/share/axoncode/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your axoncode
config.
...
axoncode zen is a list of models provided by the axoncode team that have been
tested and verified to work well with axoncode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with axoncode zen.
:::

1. Run `axoncode auth login`, select axoncode, and head to [axoncode.ai/auth](https://axoncode.ai/auth).
...
3. Paste your API key.
   ...
   │  axoncode
   │
   ●  Create an api key at https://axoncode.ai/auth
   │
   ◆  Enter your API key
   │  _
   └
...
It works like any other provider in axoncode. And is completely optional to use
it.
...
To use Amazon Bedrock with axoncode:
...
1. You'll need either to set one of the following environment variables:
   ...
   Once you have one of the above, set it while running axoncode.

   ```bash
   AWS_ACCESS_KEY_ID=XXX axoncode
   ```
...
We recommend signing up for [Claude Pro](https://www.anthropic.com/news/claude-pro) or [Max](https://www.anthropic.com/max), it's the most cost-effective way to use axoncode.

Once you've signed up, run `axoncode auth login` and select Anthropic.
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for axoncode to work properly.
   :::

3. Run `axoncode auth login` and select **Azure**.
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX axoncode
   ```
...
2. Run `axoncode auth login` and select **Cerebras**.
...
2. Run `axoncode auth login` and select **DeepSeek**.
...
2. Run `axoncode auth login` and select **Fireworks AI**.
...
To use your GitHub Copilot subscription with axoncode:
...
1. Run `axoncode auth login` and select GitHub Copilot.
...
2. Run `axoncode auth login` and select Groq.
...
You can configure axoncode to use local models through LM Studio.

```json title="axoncode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://axoncode.ai/config.json",
  "provider": {
    "lmstudio": {
...
2. Run `axoncode auth login` and select **Moonshot AI**.
...
You can configure axoncode to use local models through Ollama.

```json title="axoncode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://axoncode.ai/config.json",
  "provider": {
    "ollama": {
...
2. Run `axoncode auth login` and select OpenAI.
...
axoncode zen is a list of tested and verified models provided by the axoncode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>axoncode zen</a>** and click **Create API Key**.

2. Run `axoncode auth login` and select **axoncode zen**.
...
2. Run `axoncode auth login` and select OpenRouter.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your axoncode config.
...
5. You can also customize them through your axoncode config. Here's an example of specifying a provider

   ```json title="axoncode.json"
   {
     "$schema": "https://axoncode.ai/config.json",
     "provider": {
       "openrouter": {
...
2. Run `axoncode auth login` and select **Together AI**.
...
For a limited time, you can use xAI's Grok Code for free with axoncode.

:::tip
Grok Code is available for free for a limited time on axoncode.
:::

1. Make sure you are on the latest version of axoncode.
...
2. Run `axoncode auth login` and select **Z.AI**.
...
To add any **OpenAI-compatible** provider that's not listed in `axoncode auth login`:

:::tip
You can use any OpenAI-compatible provider with axoncode. Most modern AI providers offer OpenAI-compatible APIs.
:::

1. Run `axoncode auth login` and scroll down to **Other**.
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need configure it in axoncode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `axoncode.json` file in your project directory:

   ```json title="axoncode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://axoncode.ai/config.json",
     "provider": {
       "myprovider": {
...
1. **Check the auth setup**: Run `axoncode auth list` to see if the credentials
...
2. For custom providers, check the axoncode config and:
   - Make sure the provider ID used in `axoncode auth login` matches the ID in your axoncode config.
