...
Create `.axoncode/command/test.md`:

```md title=".axoncode/command/test.md"
---
...
You can add custom commands through the axoncode config or by creating markdown files in the `command/` directory.

---

### JSON

Use the `command` option in your axoncode [config](/docs/config):

```json title="axoncode.jsonc" {4-12}
{
...
- Global: `~/.config/axoncode/command/`
- Per-project: `.axoncode/command/`

```markdown title="~/.config/axoncode/command/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".axoncode/command/component.md"
---
...
```md title=".axoncode/command/analyze-coverage.md"
---
...
```md title=".axoncode/command/review-changes.md"
---
...
```md title=".axoncode/command/review-component.md"
---
...
axoncode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
