...
There are two types of agents in axoncode; primary agents and subagents.
...
axoncode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
axoncode comes with one built-in subagent, **General**. We'll look at this below.

---

## Built-in

axoncode comes with two built-in primary agents and one built-in subagent.
...
Configure agents in your `axoncode.json` config file:
...
- Global: `~/.config/axoncode/agent/`
- Per-project: `.axoncode/agent/`

```markdown title="~/.config/axoncode/agent/review.md"
---
...
If no temperature is specified, axoncode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global axoncode config and the project specific config.
...
Configure permissions globally in axoncode.json.
...
:::tip
Run `axoncode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/sst/axoncode).
:::
...
```markdown title="~/.config/axoncode/agent/docs-writer.md"
---
...
```markdown title="~/.config/axoncode/agent/security-auditor.md"
---
