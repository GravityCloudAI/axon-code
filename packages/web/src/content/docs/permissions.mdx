...
By default, axoncode **allows all operations** without requiring explicit approval.
...
Permissions are configured in your `axoncode.json` file under the `permission` key. Here are the available options.
