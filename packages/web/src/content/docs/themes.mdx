...
With axoncode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, axoncode uses our own `axoncode` theme.
...
axoncode comes with several built-in themes.
...
- Want axoncode to match their terminal's appearance
...
axoncode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/axoncode/themes/*.json` or `$XDG_CONFIG_HOME/axoncode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.axoncode/themes/*.json`
4. **Current working directory** - Defined in `./.axoncode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/axoncode/themes
vim ~/.config/axoncode/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .axoncode/themes
vim .axoncode/themes/my-theme.json
```
...
{
  "$schema": "https://axoncode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
