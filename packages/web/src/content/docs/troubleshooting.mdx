...
To debug any issues with axoncode, you can check the logs or the session data
...
- **macOS/Linux**: `~/.local/share/axoncode/log/`
- **Windows**: `%USERPROFILE%\.local\share\axoncode\log\`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `axoncode --log-level DEBUG`.

---

### Storage

axoncode stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/axoncode/`
- **Windows**: `%USERPROFILE%\.local\share\axoncode`
...
If you're experiencing issues with axoncode:

1. **Report issues on GitHub**

   The best way to report bugs or request features is through our GitHub repository:

   [**github.com/sst/axoncode/issues**](https://github.com/sst/axoncode/issues)
...
2. **Join our Discord**
   ...
   [**axoncode.ai/discord**](https://axoncode.ai/discord)
...
### axoncode won't start

1. Check the logs for error messages
2. Try running with `--print-logs` to see output in the terminal
3. Ensure you have the latest version with `axoncode upgrade`

---

### Authentication issues

1. Try re-authenticating with `axoncode auth login <provider>`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   ```bash
   rm -rf ~/.local/share/axoncode
   ```

3. Re-authenticate with your provider:
   ```bash
   axoncode auth login <provider>
   ```
...
If you encounter API call errors, this may be due to outdated provider packages. axoncode dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   ```bash
   rm -rf ~/.cache/axoncode
   ```

2. Restart axoncode to reinstall the latest provider packages

This will force axoncode to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
axoncode will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
