...
:::caution
Modes are now configured through the `agent` option in the axoncode config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in axoncode allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the axoncode config.
...
axoncode comes with two built-in modes.
...
Configure modes in your `axoncode.json` config file:
...
- Global: `~/.config/axoncode/mode/`
- Project: `.axoncode/mode/`

```markdown title="~/.config/axoncode/mode/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global axoncode config and the project specific config.
...
Create mode files in `.axoncode/mode/` for project-specific modes or `~/.config/axoncode/mode/` for global modes:

```markdown title=".axoncode/mode/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/axoncode/mode/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
