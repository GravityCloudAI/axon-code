...
function findBinary() {
  ...
  const packageName = `axoncode-${platform}-${arch}`
  const binary = platform === "windows" ? "axoncode.exe" : "axoncode"
...
async function regenerateWindowsCmdWrappers() {
  ...
  try {
    ...
    const cmd = `npm rebuild axoncode-ai --ignore-scripts${isGlobal ? " -g" : ""}`
    const opts = {
      stdio: "inherit",
      shell: true,
    ...
    console.error("npm rebuild failed. You may need to manually run: npm rebuild axoncode-ai --ignore-scripts")
  }
}
...
async function main() {
  try {
    ...
    const binScript = path.join(__dirname, "bin", "axoncode")
    ...
    console.log(`axoncode binary symlinked: ${binScript} -> ${binaryPath}`)
  } catch (error) {
    console.error("Failed to create axoncode binary symlink:", error.message)
    process.exit(1)
  }
}
