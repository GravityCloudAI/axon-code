...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    // Removed loading of axoncode.json and axoncode.jsonc to prevent overriding hardcoded provider
    // for (const file of ["axoncode.jsonc", "axoncode.json"]) {
    ...
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        ...
        const wellknown = await fetch(`${key}/.well-known/axoncode`).then((x) => x.json())
    ...
    const directories = [
      ...
      ...(await Array.fromAsync(
        Filesystem.up({ targets: [".axoncode"], start: Instance.directory, stop: Instance.worktree }),
      )),
    ]
    ...
    if (Flag.axoncode_PERMISSION) {
      result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.axoncode_PERMISSION))
    }
  ...
  async function loadCommand(dir: string) {
    ...
    for await (const item of COMMAND_GLOB.scan({ absolute: true, followSymlinks: true, dot: true, cwd: dir })) {
      ...
      const name = (() => {
        const patterns = ["/.axoncode/command/", "/command/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for await (const item of AGENT_GLOB.scan({ absolute: true, followSymlinks: true, dot: true, cwd: dir })) {
      ...
      const agentFolderPath = item.includes("/.axoncode/agent/")
        ? item.split("/.axoncode/agent/")[1]
        : item.includes("/agent/")
          ? item.split("/agent/")[1]
          : agentName + ".md"
  ...
  export const Keybinds = z
    .object({
      ...
      code_review: z.string().optional().default("ctrl+y,<leader>y").describe("Code review"),
  ...
  export const Info = z
    .object({
      ...
      command: z
        ...
        .optional()
        .describe("Command configuration, see https://axoncode.ai/docs/commands"),
      watcher: z
        .object({
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://axoncode.ai/docs/agent"),
      provider: z
        .record(
          z.string(),
  ...
  export const global = lazy(async () => {
    ...
    // Hardcoded MatterAI provider configuration
    const matteraiProvider = {
      matterai: {
    ...
    await import(path.join(Global.Path.config, "config"), {
      ...
      .then(async (mod) => {
        ...
        result["$schema"] = "https://axoncode.ai/config.json"
  ...
  async function load(text: string, configFilepath: string) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema) {
        parsed.data.$schema = "https://axoncode.ai/config.json"
