...
declare global {
  const axoncode_TUI_PATH: string
}

if (typeof axoncode_TUI_PATH !== "undefined") {
  await import(axoncode_TUI_PATH as string, {
    with: { type: "file" },
  })
}
...
export const TuiCommand = cmd({
  command: "$0 [project]",
  describe: "start axoncode tui",
  builder: (yargs) =>
    yargs
      .positional("project", {
        type: "string",
        describe: "path to start axoncode in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
      ...
      const result = await bootstrap(cwd, async () => {
        ...
        const proc = Bun.spawn({
          ...
          env: {
            ...process.env,
            CGO_ENABLED: "0",
            axoncode_SERVER: server.url.toString(),
          },
          onExit: () => {
            server.stop()
          },
        ...
        ;(async () => {
          ...
          if (config.autoupdate === false || Flag.axoncode_DISABLE_AUTOUPDATE) return
...
/**
 * Get the correct command to run axoncode CLI
