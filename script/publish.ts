...
const version = await (async () => {
  ...
  const [major, minor, patch] = (await $`gh release list --limit 1 --json tagName --jq '.[0].tagName'`.text())
    ...
    .split(".")
    .map((x) => Number(x) || 0)
  const t = process.env["axoncode_BUMP"]?.toLowerCase()
...
console.log("\n=== axoncode ===\n")
await import(`../packages/axoncode/script/publish.ts`)
...
if (!snapshot) {
  ...
  const previous = await fetch("https://api.github.com/repos/sst/axoncode/releases/latest")
    .then((res) => {
  ...
  const commits = await fetch(`https://api.github.com/repos/sst/axoncode/compare/${previous}...HEAD`)
    .then((res) => res.json())
  ...
  await $`gh release create v${version} --title "v${version}" --notes ${notes} ./packages/axoncode/dist/*.zip`
}
if (snapshot) {
